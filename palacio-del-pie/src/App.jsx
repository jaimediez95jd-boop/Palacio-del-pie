...
const USUARIOS = [
  ...
  { nombre: "Dr. Diez Saralgui", icono: "👨‍⚕️" },
...
function exportCSV(pacientes) {
  ...
  const rows = pacientes.map(p => [
    p.nombre, p.apellidos, p.numHistoria, p.edad, p.telefono, p.email,
    p.cirugia, p.codigoCirugia, p.fechaCirugia ? formatDate(p.fechaCirugia) : "",
...
export default function App() {
  ...
  const [usuario, setUsuario] = useState(() => sessionStorage.getItem("pdp-usuario") || "");
  ...
  const filtrados = pacientes.filter(p => {
    ...
    return txt.includes(filtro.toLowerCase())
      && (filtroPago === "todos" || (filtroPago === "pagado" ? p.pagado : !p.pagado))
      && (filtroEstado === "todos" || p.estado === filtroEstado)
      && (filtroCirujano === "todos" || p.creadoPor === filtroCirujano);
  });
    ...
    <div style={S.loginWrap}>
      <div style={S.loginCard}>
        ...
        <div style={S.usuarioGrid}>
          {USUARIOS.map(u => (
            <button key={u.nombre} style={S.usuarioBtn} onClick={() => setUsuarioAndSave(u.nombre)}>
              <span style={{ fontSize: 28 }}>{u.icono}</span>
              <span style={{ fontSize: 13, fontWeight: 700 }}>{u.nombre}</span>
            </button>
          ))}
        </div>
    ...
    <div style={S.app}>
      ...
      <header style={S.header}>
        <div style={S.headerLeft}>
          ...
          <span style={S.brandH}>Palacio del Pie</span>
      ...
      <main style={S.main}>
        ...
        {vista === "lista" && (<>
          <div style={S.statsRow}>
            {[
              { label: "Total pacientes", value: stats.total, color: "#6366f1" },
              { label: "Programados", value: stats.programados, color: "#f59e0b" },
              { label: "Operados / Alta", value: stats.operados, color: "#10b981" },
              { label: "Pago pendiente", value: stats.pendientesPago, color: "#ef4444" },
            ].map(s => (
          ...
          <div style={S.toolbar}>
            ...
            <select style={S.select} value={filtroCirujano} onChange={e => setFiltroCirujano(e.target.value)}>
              <option value="todos">Todos los cirujanos</option>
              {CIRUJANOS.map(c => <option key={c}>{c}</option>)}
            </select>
        ...
        {vista === "detalle" && selected && (
          <div style={S.formCard}>
            ...
            <div style={S.detalleGrid}>
              <DetalleItem label="Nº Historia" value={selected.numHistoria} accent />
              <DetalleItem label="Edad" value={selected.edad ? `${selected.edad} años` : null} />
              <DetalleItem label="Teléfono" value={selected.telefono} />
              <DetalleItem label="Email" value={selected.email} />
              <DetalleItem label="Cirugía" value={selected.cirugia} accent />
              <DetalleItem label="Código" value={selected.codigoCirugia} />
              <DetalleItem label="Fecha cirugía" value={formatDate(selected.fechaCirugia)} />
              <DetalleItem label="Estado" value={selected.estado} />
              <DetalleItem label="Cirujano" value={selected.creadoPor} />
              <DetalleItem label="Pago" value={selected.pagado ? "✅ Pagado" : "⏳ Pendiente"} />
        ...
        {isForm && (
          <div style={S.formCard}>
            <div style={S.formHeader}>
              <button style={S.backBtn} onClick={() => setVista("lista")}>← Volver</button>
              <h2 style={S.formTitle}>{vista === "nuevo" ? "Nuevo paciente" : "Editar paciente"}</h2>
            </div>
            ...
            <div style={S.formGrid}>
              ...
              <div style={S.fieldWrap}>
                <label style={S.label}>Cirujano responsable</label>
                <select style={S.input} value={form.creadoPor || usuario} onChange={e => setForm({ ...form, creadoPor: e.target.value })}>
            ...
            <div style={S.formGrid}>
              ...
              <div style={S.fieldWrap}>
                <label style={S.label}>Estado pago</label>
                <button style={{ ...S.toggleBtn, ...(form.pagado ? S.toggleOn : S.toggleOff) }} onClick={() => setForm({ ...form, pagado: !form.pagado })}>
...
const S = {
  ...
  pagadoOn: { background: "#dcfce7", color: "#15803d" },
  pagadoOff: { background: "#fee2e2", color: "#dc2626" },
  ...
  toggleOff: { background: "#fee2e2", color: "#dc2626" },
